...
class ReportTemplate(BaseModel):
    ...
    report_type = models.CharField(
        max_length=20, choices=REPORT_TYPE_CHOICES, verbose_name=_("Tipo de Reporte")
    )
    format = models.CharField(
        max_length=10, choices=FORMAT_CHOICES, default="pdf", verbose_name=_("Formato")
    )
    template_config = models.JSONField(
        default=dict, blank=True, verbose_name=_("Configuración de Plantilla")
    )
...
class ScheduledReport(BaseModel):
    ...
    frequency = models.CharField(
        max_length=20, choices=FREQUENCY_CHOICES, verbose_name=_("Frecuencia")
    )
    recipients = models.JSONField(default=list, verbose_name=_("Destinatarios"))
    parameters = models.JSONField(
        default=dict, blank=True, verbose_name=_("Parámetros")
    )
    last_generated = models.DateTimeField(
        null=True, blank=True, verbose_name=_("Última Generación")
    )
    next_generation = models.DateTimeField(verbose_name=_("Próxima Generación"))
    is_active = models.BooleanField(default=True, verbose_name=_("Activo"))
...
class GeneratedReport(BaseModel):
    ...
    parameters = models.JSONField(
        default=dict, blank=True, verbose_name=_("Parámetros Utilizados")
    )
    file_path = models.CharField(
        max_length=500, blank=True, verbose_name=_("Ruta del Archivo")
    )
    file_size = models.BigIntegerField(
        null=True, blank=True, verbose_name=_("Tamaño del Archivo (bytes)")
    )
    generation_time = models.DurationField(
        null=True, blank=True, verbose_name=_("Tiempo de Generación")
    )
...
class ReportParameter(BaseModel):
    ...
    display_name = models.CharField(
        max_length=100, verbose_name=_("Nombre de Visualización")
    )
    ...
    default_value = models.TextField(blank=True, verbose_name=_("Valor por Defecto"))
    validation_rules = models.JSONField(
        default=dict, blank=True, verbose_name=_("Reglas de Validación")
    )
    choices = models.JSONField(
        default=list, blank=True, verbose_name=_("Opciones de Selección")
    )
