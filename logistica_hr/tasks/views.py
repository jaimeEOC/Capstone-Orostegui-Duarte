...
def my_tasks(request):
    ...
    qs = qs.order_by('-created_at', '-due_date', 'priority')
...
class TaskMiniForm(forms.ModelForm):
    ...
    def __init__(self, *args, **kwargs):
        # Extraer 'user' de kwargs antes de pasar a super()
...
def create_task(request):
    if request.method == "POST":
        form = TaskMiniForm(request.POST, user=request.user)
        # Setear assigned_by antes de is_valid()
        ...
        form = TaskMiniForm(user=request.user)
