...
class PerformanceMetric(BaseModel):
    ...
    metric_type = models.CharField(
        max_length=20, choices=METRIC_TYPE_CHOICES, verbose_name=_("Tipo de Métrica")
    )
...
class EmployeePerformance(BaseModel):
    ...
    actual_value = models.DecimalField(
        max_digits=10, decimal_places=2, verbose_name=_("Valor Real")
    )
...
class DailyWorkLog(BaseModel):
    ...
    total_break_time = models.DurationField(
        default=0, verbose_name=_("Tiempo Total de Descanso")
    )
    packages_processed = models.IntegerField(
        default=0, verbose_name=_("Paquetes Procesados")
    )
    trucks_received = models.IntegerField(
        default=0, verbose_name=_("Camiones Recibidos")
    )
    trucks_dispatched = models.IntegerField(
        default=0, verbose_name=_("Camiones Despachados")
    )
    ...
    safety_incidents = models.IntegerField(
        default=0, verbose_name=_("Incidentes de Seguridad")
    )
    notes = models.TextField(blank=True, verbose_name=_("Notas"))
...
class PerformanceEvaluation(BaseModel):
    ...
    strengths = models.TextField(blank=True, verbose_name=_("Fortalezas"))
    areas_for_improvement = models.TextField(
        blank=True, verbose_name=_("Áreas de Mejora")
    )
    ...
    evaluation_date = models.DateTimeField(
        auto_now_add=True, verbose_name=_("Fecha de Evaluación")
    )
    ...
    def __str__(self):
        return (
            f"{self.employee} - {self.get_evaluation_type_display()} - {self.end_date}"
        )
